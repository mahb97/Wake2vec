...
```
~124s/step — the CPU offload tax is real
~29 steps/hour
~115 steps per 4hr session
~26 sessions to complete 3,000 steps
VRAM during training: 12.6 / 15.0 GB (2.4 GB headroom)
```

The Qween takes her sweet time but I have no deadline, and infinite 3-4hr blocks when Colab allows, every step counts on this one though.

Might change EVAL_STEPS from 200 → 100 (at 124s/step, 200 steps = ~7 hours between evals, would be nice to get at least one reading per session).
