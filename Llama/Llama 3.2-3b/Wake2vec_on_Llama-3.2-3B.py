# -*- coding: utf-8 -*-
"""Wake2vec_on_Llama-3.2-3B.py

# Wake2Vec P1: Llama-3.2-3B Embedding-Only with Gradient Masking
...
Llama-3.2-3B vocabulary and initialized on a sphere. Only the new Wake
...
Based on the Llama-3.2-1B script. Main differences:
- 3B model: 28 layers, hidden_size=3072, 24 attn heads, 8 KV heads
- Same vocab (128,256) so Wake injection is identical
- ~2GB heavier at 4-bit but still well within T4 VRAM (~6GB total)
- Dropped seq_len to 256 to be safe on slower training
  (bump back to 512 if VRAM allows and you want longer context)

VRAM estimate: ~5-6GB. should not OOM. ever. hopefully.
"""
...
# output dir
RUN_DIR = Path("/content/drive/MyDrive/wake2vec_llama3b_p1")
LOCAL_RUN = Path("/content/runs/wake2vec_llama3b_p1")
...
BATCH_SIZE = 1
GRAD_ACCUM = 16
SEQ_LEN = 256               # conservative. bump to 512 if VRAM is chill
SAVE_STEPS = 50              # colab disconnects are real and they hurt
LOG_STEPS = 50
EVAL_STEPS = 200
EMB_SNAP_STEPS = 50

# Resume — set to None for fresh start, or point at a sentry checkpoint
RESUME_FROM = None
# RESUME_FROM = SENTRY / "checkpoint-200"

print("WAKE2VEC P1: Llama-3.2-3B CONFIG")
...
print("=" * 60)
print("WAKE2VEC P1: Llama-3.2-3B EMBEDDING-ONLY")
print("=" * 60)
...
if state_files:
    ...
    if train_data:
        ...
        plt.xlabel('Step')
        plt.ylabel('Loss')
        plt.title('Wake2Vec P1: Llama-3.2-3B Loss Curve')
        plt.legend()
        plt.grid(True, alpha=0.3)

        plot_path = RUN_DIR / "p1_llama3b_loss_curve.png"
...
if has_pre:
    E_pre_all = torch.load(pre_path, map_location="cpu").to(torch.float32).numpy()
...
fig.suptitle("Wake2Vec P1 Llama-3.2-3B -- Embedding Analysis", fontsize=14, fontweight="bold")
...
plt.tight_layout()
analysis_plot = RUN_DIR / "p1_llama3b_analysis.png"
...
summary_path = RUN_DIR / "p1_llama3b_summary.json"
