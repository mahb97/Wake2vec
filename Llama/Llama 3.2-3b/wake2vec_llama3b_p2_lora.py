# -*- coding: utf-8 -*-
"""Wake2Vec Phase 2: LoRA Fine-Tune (Llama-3.2-3B)

# Wake2Vec Phase 2: LoRA Fine-Tune with Frozen P1 Embeddings

**Model:** meta-llama/Llama-3.2-3B (4-bit quantized)
**Hardware:** Google Colab T4 GPU (2026.05)
**P1 Source:** wake2vec_llama3b_p1/full_checkpoints/step_0300/ (best val, gradient-masked embeddings)

## Overview

Phase 2 loads the embedding weights from the Llama 3.2-3B P1 run
(3000 steps, gradient-masked, U-curve closed back at val ~7.08) and
freezes them entirely. LoRA adapters are applied to attention (q, k, v)
and MLP (gate, up, down) projections. The model learns to use the
Wake-adapted embeddings through attention and MLP adaptation rather than
further embedding modification.

P1 best val was at step 300 (val 6.68). The U-curve then climbed back
to 7.08 by step 3000 (pure memorisation). Loading from step 300 means
the embeddings are at their semantically meaningful peak and least
overfit by validation metric. 
...
| LR | 2e-4 | 2e-5 |
| Steps | 3000 | 3000 |
| Batch | 1 x 16 | 2 x 8 (3B is bigger, reduced batch) |

## Colab 2026.05 compatibility
- Triton shim for bnb / triton 3.x (harmless no-op if already patched)
- bitsandbytes >= 0.46.1 required
- `dtype` not `torch_dtype` (transformers 5.x)
- **SentryMirror fix included from the start** (has_weights(dst) verification
  before skipping 
...
# install as needed
# !pip install -q -U bitsandbytes peft accelerate scikit-learn scipy
...
# P1 source (Llama-3.2-3B, embedding-only, best val @ step 300)
P1_SOURCE = Path("/content/drive/MyDrive/wake2vec_llama3b_p1/full_checkpoints/step_0300")

# P2 outputs 
RUN_DIR = Path("/content/drive/MyDrive/wake2vec_llama3b_p2_lora")
LOCAL_RUN = Path("/content/runs/wake2vec_llama3b_p2_lora")
...
print("Llama-3.2-3B LoRA CONFIG")
    ...
    def on_save(self, args, state, control, **kw):
        try:
            ...
            if not has_weights(ck):
                print(f"[SENTRY] {ck.name}: local has no weights yet, so skipping this")
                return
            ...
            if not has_weights(dst):
                print(f"[SENTRY] {ck.name}: WARNING weights are missing after copy")
            else:
...
print("P2 Llama-3.2-3B LoRA FINE-TUNE")
...
if state_files:
    ...
    if train_data:
        ...
        plt.xlabel('Step')
        plt.ylabel('Loss')
        plt.title('Wake2Vec P2: Llama-3.2-3B LoRA Loss Curve')
        plt.legend()
        plt.grid(True, alpha=0.3)

        plot_path = RUN_DIR / "p2_llama3b_loss_curve.png"
...
fig.suptitle("Wake2Vec P2 Llama-3.2-3B LoRA -- Embedding Analysis", fontsize=14, fontweight="bold")
...
if has_pre:
    ...
    ax.set_xlabel("Cosine similarity (pre -> post)"); ax.set_ylabel("Frequency")
...
ax.legend(fontsize=8, markerscale=10); ax.grid(True, alpha=0.3)
...
plt.tight_layout()
analysis_plot = RUN_DIR / "p2_llama3b_analysis.png"
...
summary_path = RUN_DIR / "p2_llama3b_summary.json"
summary_path.write_text(json.dumps(report, indent=2))
print(f"\n[SUMMARY] Saved to {summary_path}")
print(json.dumps(report, indent=2))

"""## wake generation and temp sweep"""
