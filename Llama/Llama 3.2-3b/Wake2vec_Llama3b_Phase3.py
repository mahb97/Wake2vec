# -*- coding: utf-8 -*-
"""Wake2Vec Phase 3: Morpheme-Compositional Alignment (Llama-3.2-3B)

# Wake2Vec Phase 3: Morpheme-Compositional Alignment, with strong Lambdas

**Model:** meta-llama/Llama-3.2-3B (4-bit quantized)
**Hardware:** Google Colab T4 GPU
**Source:** 3B P2 step 100 checkpoint (best-val, val 5.33)

## Methodology

Based on TinyLlama and Llama 3.2-1B P3 findings:
- TinyLlama P3 (weak lambdas: morph=0.1, device=0.05): L_morph pinned
  at 0.0002, L_device random walk 0.17-0.24 across 1400 steps.
- TinyLlama P3b (strong lambdas: morph=50, device=2.0): L_device at 12%
  of total loss but still didn't move across 800 steps. Structural null.
- Llama 1B P3 (strong lambdas direct): L_morph=0.0007, never moved.
  Cross-architecture null confirmed.

3B P2 produced the cleanest plateau in the project: val 5.33 to four decimal 
places across 5 consecutive evals (crystallised, not plateaued). The smaller-model 
paradox now has a numerical signature at the LM loss level. P3 tests whether 
morpheme-compositional alignment can break the 5.33 ceiling that LoRA couldn't,
or whether the ceiling is structural to the 128K-vocab Llama at scale.

We skip weak lambdas (confirmed null across two architectures) and go
directly to strong lambdas. Same null = paradox holds at the loss
geometry level. Movement = a route past the 128K-vocab dominance.

- Source: 3B P2 step 100 (best val 5.33, tied with steps 200-500)
- Strong lambdas: morph=50.0, device=2.0
- LR: 2e-5
- 1000 steps max, early stop patience 5
...
# P3 source is the best-val checkpoint from 3B P2 run 
# 3B P2 val crystallised at 5.33 across all 5 evals (step 100, 200, 300, 400, 500). Step 100 is the earliest of the tied values. Q: can morpheme-compositional alignment break the 5.33 ceiling that LoRA couldn't?
P2_SOURCE = Path("/content/drive/MyDrive/wake2vec_llama3b_p2_lora/full_checkpoints/step_0100")

# outputs
RUN_DIR = Path("/content/drive/MyDrive/wake2vec_llama3b_p3_morpheme")
LOCAL_RUN = Path("/content/runs/wake2vec_llama3b_p3_morpheme")
...
# early stopping 
EARLY_STOP_PATIENCE = 5     # tighter for short run
...
# load P2 trained embeddings (step 100, best val 5.33, tied with 200-500)
...
# get P2 step 100 embeddings (best-val source, tied with steps 200-500)
...
# unfreeze Wake embed rows via gradient masking
...
"""## Cell 12: Loss Curves (with component breakdown)"""
...
fig.suptitle("Wake2Vec P3 Llama-3.2-3B -- Loss Curves", fontsize=14, fontweight="bold")
...
plt.tight_layout()
plot_path = RUN_DIR / "p3_llama3b_loss_curves.png"
...
if has_pre:
    ...
    print("embed drift (pre vs post refinement)")
    print(f"  Base tokens:")
...
print("nearest neighbour sanity checks")
...
# analysis plots
fig, axes = plt.subplots(2, 3, figsize=(18, 10))
fig.suptitle("Wake2Vec P3 Llama-3.2-3B -- Embedding Analysis", fontsize=14, fontweight="bold")
...
plt.tight_layout()
analysis_plot = RUN_DIR / "p3_llama3b_analysis.png"
...
def analyze_morpheme_group(group, embed_matrix, tokenizer):
    ...
    # Ccosine similarity of each direction to the mean direction
...
# sort by direction consistency (higher = more consistent morpheme vector)
...
summary_path = RUN_DIR / "p3_llama3b_summary.json"
...
# generate_wake("Triverrun, past Eve and Adam's,", num_return_sequences=3, temperature=0.9)
