...
class Config:
    # paths (Colab + Drive)
    fw_text_path = "/content/drive/MyDrive/wake2vec/FW_TEXT.txt"
    wake_lexicon_path = "/content/drive/MyDrive/wake2vec/wake_lexicon.txt"
    output_dir = "/content/grassmann_experiment"
    drive_output_dir = "/content/drive/MyDrive/wake2vec/grassmann_experiment"

    # model architecture (matched to Zhang Chong's setup)
    vocab_size = 8192          # character-level, set after tokenizer init
    d_model = 256              
    n_layers = 6             
    ...
    warmup_steps = 500
    grad_clip = 1.0
    eval_interval = 100        # steps between evals
    save_interval = 100       # steps between checkpoint saves
...
class GrassmannMixingBlock(nn.Module):
    ...
    def __init__(self, d_model, r=32, windows=None, dropout=0.1):
        ...
        # project plucker coords back to d_model
    ...
    def forward(self, h):
        ...
        # multi-scale pairing + Plucker encoding, accumulate grassmann features per position
        ...
        for delta in self.windows:
            ...
            # Actually for causal LM we pair t with t-delta (look back), so position t sees positions t-delta (past context)
            if delta > 0:
...
class GrassmannTransformerBlock(nn.Module):
    ...
    NOTE: No residual around grass() because the gated fusion
    (alpha * h + (1-alpha) * g) already acts as the skip connection.
    When alpha=1, output = h (identity). Adding x + grass(x) would
    double-count the residual path.
    """
    ...
    def forward(self, x):
        x = self.grass(x)           # gating handles residual internally
        x = x + self.ffn(self.ln2(x))
        return x
...
def train_model(model, model_name, train_loader, val_loader, cfg, tokenizer):
    ...
    # mirror to Drive (local-first write pattern)
    if hasattr(cfg, "drive_output_dir") and cfg.drive_output_dir:
        drive_save_dir = os.path.join(cfg.drive_output_dir, model_name)
        os.makedirs(drive_save_dir, exist_ok=True)
        for fname in ["best.pt", "final.pt", "history.json"]:
            src = os.path.join(save_dir, fname)
            if os.path.exists(src):
                shutil.copy2(src, os.path.join(drive_save_dir, fname))
        print(f"  >> Mirrored to Drive: {drive_save_dir}")
...
def run_experiment(mode="train"):
    ...
    # also save tokenizer to Drive
    if hasattr(cfg, "drive_output_dir") and cfg.drive_output_dir:
        os.makedirs(cfg.drive_output_dir, exist_ok=True)
        tokenizer.save(os.path.join(cfg.drive_output_dir, "tokenizer.json"))

    # tokenize
    all_ids = tokenizer.encode(raw_text)
    print(f"  Total tokens: {len(all_ids):,}")

    # train/val split (90/10), split into non-overlapping blocks to prevent train/val leakage
    block_size = cfg.max_seq_len + 1  # +1 for target
    blocks = []
    ...
    # flatten back to id sequences
    train_ids = [idx for block in train_blocks for idx in block]
    val_ids = [idx for block in val_blocks for idx in block]
    ...
    if mode == "train":
        ...
        for name, model in [("transformer", transformer_model), ("grassmann", grassmann_model)]:
            ...
            # fallback to Drive
            if not os.path.exists(ckpt) and hasattr(cfg, "drive_output_dir"):
                ckpt = os.path.join(cfg.drive_output_dir, name, "best.pt")
            if not os.path.exists(ckpt) and hasattr(cfg, "drive_output_dir"):
                ckpt = os.path.join(cfg.drive_output_dir, name, "final.pt")
            if not os.path.exists(ckpt):
    ...
    # Evaluate: perplexity + stylometry 
        ...
        if hasattr(cfg, "drive_output_dir") and cfg.drive_output_dir:
            os.makedirs(cfg.drive_output_dir, exist_ok=True)
            shutil.copy2(res_path, os.path.join(cfg.drive_output_dir, "eval_results.json"))
            for name in ["transformer", "grassmann"]:
                gen_src = os.path.join(cfg.output_dir, name, "generations.txt")
                if os.path.exists(gen_src):
                    drive_name_dir = os.path.join(cfg.drive_output_dir, name)
                    os.makedirs(drive_name_dir, exist_ok=True)
                    shutil.copy2(gen_src, os.path.join(drive_name_dir, "generations.txt"))
            print(f"Results mirrored to Drive: {cfg.drive_output_dir}")
