# -*- coding: utf-8 -*-
"""Wake2Vec Phase 3: Morpheme-Compositional Alignment (Mistral 7B v0.3), Strong Lambdas
...
**Source:** Mistral P2 best-val (step 600, val 4.705), SPLIT across two paths:
  - LoRA adapter + tokenizer from wake2vec_mistral7b_p2_lora/full_checkpoints/step_0600
  - Embeddings from wake2vec_mistral7b_p2_lora/embeddings_frozen.pt (P2 saved them once)

Every other P3 so far has been a geometric null:
- TinyLlama P3 (weak) and P3b (strong): L_morph pinned ~0.0002, L_device
  random walk 0.17-0.24, no movement across 2000+ combined steps.
- Llama 1B P3 (strong) and Llama 3B P3 (strong): the same null, cross-architecture.

The Wake region is near-perfectly isotropic (0.998) in every model, so there is 
no pre-existing structure for the auxiliary losses to amplify. The device triplet 
loss asks embeddings to cluster by word-formation process, but on a uniform sphere 
there is no preferential direction for clusters to form.

Mistral's P1 analysis is the ONLY one below the 0.998 attractor: Wake isotropy 0.995, 
new-new pairwise cosine 0.0528 (an order of magnitude above every other model), Wake top-1 PC
0.0228. Mistral reorganised its Wake embeddings the deepest in the lineup
(drift 0.485) and deposited measurable internal structure. So Mistral is the
first P3 where the auxiliary losses have something to latch onto.

- L_device MOVES on Mistral where it was null everywhere else - the geometric
  null was a no-structure problem, not a wrong-objective problem. Structure was
  the missing ingredient, and the losses work when it is present.
- STILL NULL on Mistral - the strongest form of the null: even a region with
  measurable pre-existing structure cannot be amplified by these losses. The
  null is objective-structural, not data-structural, and universal.

Source: Mistral P2 step 600, strong lambdas: morph=50.0, device=2.0, LR: 2e-5, 1000 steps max, 
...
# P3 source
P2_RUN = Path("/content/drive/MyDrive/wake2vec_mistral7b_p2_lora")
ADAPTER_SOURCE = P2_RUN / "full_checkpoints" / "step_0600"   
EMB_SOURCE = P2_RUN / "embeddings_frozen.pt"                 

# P3 output 
RUN_DIR = Path("/content/drive/MyDrive/wake2vec_mistral7b_p3_morpheme_strong")
LOCAL_RUN = Path("/content/runs/wake2vec_mistral7b_p3_morpheme_strong")
...
# get tokenizer from the adapter checkpoint 
...
# get Mistral P2 frozen embeddings 
...
print("P2 embeddings loaded and manually tied (no tie_weights() call)")

# get P2 LoRA adapters 
...
class SentryMirror(TrainerCallback):
    def on_save(self, args, state, control, **kw):
        try:
            ...
            if dst.exists():
                if has_weights(dst):
                    return  # already complete, skip silently
                else:
...
fig.suptitle("Wake2Vec P3 Mistral 7B v0.3 Loss Curves", fontsize=14, fontweight="bold")
...
plt.tight_layout()
plot_path = RUN_DIR / "p3_mistral7b_loss_curves.png"
...
if has_pre:
    ...
    if abs(wake_iso_delta) < 1e-3:
        ...
        print("     from the one configuration in the lineup that had structure to move.")
    else:
        print("  >> Reads as NON-NULL: the Wake geometry moved under strong lambdas.")
        print("     First measurable geometric movement in the project.")
...
# pairwise cosine similarity distibutions 
...
# plots
fig, axes = plt.subplots(2, 3, figsize=(18, 10))
fig.suptitle("Wake2Vec P3 Mistral 7B v0.3 -- Embedding Analysis", fontsize=14, fontweight="bold")
...
plt.tight_layout()
analysis_plot = RUN_DIR / "p3_mistral7b_analysis.png"
...
print("MORPHEME-SPECIFIC ANALYSIS")
...
if has_pre:
    ...
    if paired:
        ...
        if abs(mean_delta) < 1e-3:
            ...
            print("     against the L_morph training curve before claiming it.")
...
# view for a few interesting groups
...
summary_path = RUN_DIR / "p3_mistral7b_summary.json"
summary_path.write_text(json.dumps(report, indent=2))
print(f"\n[SUMMARY] Saved to {summary_path}")
print(json.dumps(report, indent=2))

"""## wake style generation test"""
