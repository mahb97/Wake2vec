# -*- coding: utf-8 -*-
"""Wake2Vec Phase 1: Mistral-7B-v0.3 Embedding-Only Fine-Tune

# Wake2Vec P1: Mistral-7B-v0.3 Embedding-Only with Gradient Masking

**Model:** mistralai/Mistral-7B-v0.3 (4-bit quantized)
...
Mistral-7B-v0.3 vocabulary and initialized on a sphere. Only the new Wake
...
- 7B model: 32 layers, hidden_size=4096, 32 attn heads, 8 KV heads, sliding window attention
- Same vocab (128,256) so Wake injection is identical
- Similar VRAM to Llama 8B at 4-bit (~8-9GB). Sliding window attention is the architectural difference
- Starting with SEQ_LEN=512. May need to reduce to 256 or 128 if OOM
  Different attention mechanism (sliding window) — tests whether Wake embedding injection generalises beyond Llama.
...
FW_TEXT = "/content/drive/MyDrive/wake2vec/FW_TEXT.txt"
WAKE_LEXICON = "/content/drive/MyDrive/wake2vec/wake_lexicon.txt"

# outputs 
RUN_DIR = Path("/content/drive/MyDrive/wake2vec_mistral7b_p1")
LOCAL_RUN = Path("/content/runs/wake2vec_mistral7b_p1")
...
BATCH_SIZE = 1
GRAD_ACCUM = 16
SEQ_LEN = 512               # reduce to 256/128 if OOM on T4
SAVE_STEPS = 50              
LOG_STEPS = 50
EVAL_STEPS = 100
EMB_SNAP_STEPS = 50
...
print("WAKE2VEC P1: Mistral-7B-v0.3 CONFIG")
...
# load & concatenate FW text + Wake lexicon
...
# frozen LoRA r=1 (PEFT compatibility with quantized training)
peft_cfg = LoraConfig(
...
class EmbeddingSnapshot(TrainerCallback):
    """save embed weights at regular intervals."""
...
# only optimize embedding weight
...
if state_files:
    ...
    if train_data:
        ...
        plt.xlabel('Step')
        plt.ylabel('Loss')
        plt.title('Wake2Vec P1: Mistral-7B-v0.3 Loss Curve')
        plt.legend()
        plt.grid(True, alpha=0.3)

        plot_path = RUN_DIR / "p1_mistral7b_loss_curve.png"
...
fig.suptitle("Wake2Vec P1 Mistral-7B-v0.3 -- Embedding Analysis", fontsize=14, fontweight="bold")
...
plt.tight_layout()
analysis_plot = RUN_DIR / "p1_mistral7b_analysis.png"
...
summary_path = RUN_DIR / "p1_mistral7b_summary.json"
