...
1. **Pin + Restart:** Run the **Pin exact versions + AUTO-RESTART** cell once. When the runtime restarts, run the **Pin + Shim (fixed)** cell (it will confirm versions and apply the unwrap compat).
2. **GPU Probe:** Run the **Tiny GPU probe** cell; you should see: `[GPU OK] cuda:0`.
3. **Resume to TARGET:** Run the **Resume → TARGET with SentryMirror** cell. It will print something like:
...
# quick glance last 5 loss points
import json, pathlib
...
def load_state(run):
    ...
    # else newest checkpoint's trainer_state.json
