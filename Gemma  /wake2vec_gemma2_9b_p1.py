# -*- coding: utf-8 -*-
"""Wake2Vec Phase 1: Gemma 2 9B Embedding-Only Fine-Tune

# Wake2Vec P1: Gemma 2 9B Embedding-Only with Gradient Masking

**Model:** google/gemma-2-9b (4-bit quantized)
**Hardware:** Google Colab T4 GPU (2026.04)
**Training data:** Finnegans Wake corpus + Wake lexicon

## Overview

Phase 1 embedding-only fine-tuning. Wake tokens are injected into the
Gemma 2 9B vocabulary and initialised using a two-tier strategy:
...
# needs recent bitsandbytes (0.46.1)
...
BASE_VOCAB = 256000  # f me

# data
FW_TEXT = "/content/FW_TEXT.txt"
WAKE_LEXICON = "/content/wake_lexicon.txt"
MORPHEME_JSONL = "/content/wake_embedding_groups.jsonl"  # for compositional init

# output paths
RUN_DIR = Path("/content/drive/MyDrive/wake2vec_gemma2_9b_p1")
LOCAL_RUN = Path("/content/runs/wake2vec_gemma2_9b_p1")
...
BATCH_SIZE = 1
GRAD_ACCUM = 16
SEQ_LEN = 512               # Gemma 2 9B at 4-bit ~6-7GB; 512 should fit
SAVE_STEPS = 50              # frequent saves due to T4 disconnects
LOG_STEPS = 50
EVAL_STEPS = 100             
EMB_SNAP_STEPS = 50
...
print("WAKE2VEC P1: Gemma 2 9B CONFIG")
...
"""## tokenizer + Wake Vocab injection

expecting fewer Wake tokens to be added
compared to the 32K (TinyLlama, Mistral) or 128K (Llama) models.
"""
...
# Sanity check: with 256K vocab, expect <10K new tokens typically
if num_added < 5000:
    print(f"  >> NOTE: Small injection ({num_added} tokens).")
elif num_added < 20000:
    print(f"  >> Moderate injection. Comparable to Qwen 2.5-14B (152K vocab).")
else:
    print(f"  >> Large injection which is surprising for 256K vocab. Worth checking.")
...
# concatenate: FW corpus first, then lexicon
...
# tie input/output (Gemma 2 ties by default, force here for safety after resize)
if hasattr(model, "lm_head"):
    model.lm_head.weight = wte.weight

# embedding initialisation 
# compositional init for Wake words with morpheme decomposition
# and spherical init at 1.0x base radius for the rest
# Learned from TinyLlama/Llama 1B: 1.5x radius creates a norm gap
...
with torch.no_grad():
    ...
    target_radius = 1.0 * base_radius  

    # start with spherical init for aLL new tokens
    if num_added > 0:
    ...
    if num_added > 0 and os.path.exists(MORPHEME_JSONL):
      print("  loading morpheme groups for compositional init...")
        ...
        # build a map: wake_word to base_word
        word_to_base = {}
        ...
        # Load lexicon (only the missing ones get embedding rows in order added)
        # Build list of words that were actually added (in the order they were added)
        added_words = missing  # preserves insertion order from `tok.add_tokens`

        # for each Wake token that has a known base, compose the init
        for i, wake_word in enumerate(added_words):
            if i >= num_added:
                break
        ...
        print(f"  no new tokens to initialise so all Wake forms in base vocab.")
    else:
        print(f"  lol forgot to add {MORPHEME_JSONL}")
...
if num_added == 0:
    print()
    print("  >> Nothing to learn here.")
    print("     256K Gemma tokenizer covers all Wake forms natively.")
    raise SystemExit("No Wake tokens to inject.")
...
class LossMonitor(TrainerCallback):
    """warning for train/eval divergence."""
    def __init__(self):
...
if state_files:
    ...
    if train_data:
        ...
        plt.xlabel('Step')
        plt.ylabel('Loss')
        plt.title('Wake2Vec P1: Gemma 2 9B Loss Curve')
        plt.legend()
        plt.grid(True, alpha=0.3)

        plot_path = RUN_DIR / "p1_gemma2_9b_loss_curve.png"
    ...
    print("lol no trainer_state.json found")
...
if has_pre:
    ...
    print("[PRE] lol no pre-training snapshot found (skipping drift analysis)")
...
if num_new_tokens >= 2:
    ...
    t_stat = t_pval = u_stat = u_pval = cohens_d = float("nan")
...
if num_new_tokens >= 50:
    iso_new, cos_new, n_new = compute_isotropy(E_new)
else:
    iso_new = cos_new = float("nan")
    n_new = num_new_tokens
...
if has_pre and num_new_tokens > 0:
    ...
    show = min(20, num_new_tokens)
    print(f"\n  Top {show} most-changed Wake tokens:")
    for rank, idx in enumerate(wake_drift_order[:show]):
...
print(f"\n{'=' * 60}")
print("nearest neighbours (Wake tokens / base vocab)")
...
if num_new_tokens > 0:
    ...
    sample_wake_ids = list(range(BASE_VOCAB, BASE_VOCAB + min(10, num_new_tokens)))
    ...
    print("  No Wake tokens added (256K vocab covered all forms natively)")

# intrinsic dimensionality (PCA)

print(f"\n{'=' * 60}")
print("intrinsic dimensionality (PCA explained variance)")
...
if num_new_tokens >= 100:
    ...
    print(f"  Skipped — fewer than 100 new tokens ({num_new_tokens})")
    pca_base = pca_new = None
    cumvar_base = cumvar_new = None
    dim90_base = dim90_new = dim95_base = dim95_new = float("nan")
...
if num_new_tokens >= 50:
    ...
    cos_nn = cos_bn = np.array([])
    ks_stat = ks_pval = float("nan")
    print(f"  Skipped — fewer than 50 new tokens ({num_new_tokens})")

# plots

fig, axes = plt.subplots(2, 3, figsize=(18, 10))
fig.suptitle("Wake2Vec P1 Gemma 2 9B -- Embedding Analysis", fontsize=14, fontweight="bold")
...
# b) cosine similarity distributions
ax = axes[0, 1]
...
if pca_base is not None:
    ...
    ax.text(0.5, 0.5, "Too few tokens for PCA",
...
plt.tight_layout()
analysis_plot = RUN_DIR / "p1_gemma2_9b_analysis.png"
...
report = {
    ...
    "hyperparameters": {
        ...
        "embedding_init": "compositional+spherical_1.0x",
    },
    "norms": {
        ...
        "new": {"mean": float(new_norms.mean()) if num_new_tokens > 0 else None,
                "std": float(new_norms.std()) if num_new_tokens > 0 else None},
    ...
    "pairwise_cosine": {
        ...
        "new_new": {"mean": float(cos_nn.mean()) if len(cos_nn) else None,
                    "std": float(cos_nn.std()) if len(cos_nn) else None},
        "base_new": {"mean": float(cos_bn.mean()) if len(cos_bn) else None,
                     "std": float(cos_bn.std()) if len(cos_bn) else None},
        "ks_test_bb_vs_nn": {"D": float(ks_stat), "p": float(ks_pval)},
    },
    "intrinsic_dim": {
        "base_90pct": int(dim90_base) if pca_base is not None else None,
        "base_95pct": int(dim95_base) if pca_base is not None else None,
        "new_90pct": int(dim90_new) if pca_new is not None else None,
        "new_95pct": int(dim95_new) if pca_new is not None else None,
        "base_top1_var": float(pca_base.explained_variance_ratio_[0]) if pca_base is not None else None,
        "new_top1_var": float(pca_new.explained_variance_ratio_[0]) if pca_new is not None else None,
    },
    "loss": {
...
summary_path = RUN_DIR / "p1_gemma2_9b_summary.json"
