# -*- coding: utf-8 -*-
"""Wake2Vec Phase 2: LoRA Fine-Tune (Phi-3.5-mini-instruct)

# Wake2Vec Phase 2: LoRA Fine-Tune with Frozen P1 Embeddings

**Model:** microsoft/Phi-3.5-mini-instruct (4-bit quantized, instruct-tuned)
**Hardware:** Google Colab T4 GPU + CPU offload
**P1 Source:** wake2vec_phi35_p1/full_checkpoints/step_0300 

Phi is the textbook-resists datapoint. Across 3000 P1 steps its training loss
fell from 12.40 into the low 9s (it memorised the training Wake blocks) while
held-out validation never left a ~12.28 wall, a point worse than the
uniform-token baseline. 

## Overview

Phase 2 loads the embedding weights from the Phi-3.5 P1 run and freezes them
entirely. LoRA adapters are applied to attention and MLP projections. The model
learns to use the frozen Wake embeddings through attention/MLP adaptation
rather than by modifying the embeddings further.

That makes Phi's P2 the sharp end of the 2x2 controlled comparison against
Mistral (same 58% Wake share, same spherical 1.5x init, same nothing-else-held
family; only pretraining data differs, textbook vs internet). Mistral's P1
reorganised the deepest in the lineup and its P2 overfit-and-turned cleanly
(best val 4.705 @ 600). The open question for Phi is the memorisation-vs-
transformation test: with near-baseline P1 embeddings, does LoRA routing let 
Phi transform the Wake, or does it only regurgitate memorised training blocks. 

## Phi-specifics

1. Fused projections for different LoRA targets: Phi-3 fuses q/k/v into a
   single `qkv_proj` and gate/up into `gate_up_proj`. 
2. Padding gap: The Phi embedding matrix is padded to 32064 while the
   tokenizer length is 32011. P1 anchored BASE_VOCAB and the gradient mask at
   32064; P2 freezes and loads the whole P1 matrix, so the gap is already baked
   into embeddings.pt and the load is shape-driven.
3. Manual tie, no tie_weights() call: transformers 5.x has a tie_weights bug
   that crashed on Phi; setting tie_word_embeddings=False and doing the manual
   `lm_head.weight = wte.weight` achieves the same sharing without the buggy
   method.
4. trust_remote_code=True: on the model and tokenizer.

## Colab 2026 compatibility
...
import warnings
warnings.filterwarnings("ignore", message=r"The attention mask API under",
...
# P1 source 
P1_SOURCE = Path("/content/drive/MyDrive/wake2vec_phi35_p1/full_checkpoints/step_0300")

# outputs
RUN_DIR = Path("/content/drive/MyDrive/wake2vec_phi35_p2_lora")
LOCAL_RUN = Path("/content/runs/wake2vec_phi35_p2_lora")
...
LORA_ALPHA = 16
LORA_DROPOUT = 0.1
# qkv_proj  == fused q_proj+k_proj+v_proj  
# gate_up_proj == fused gate_proj+up_proj   
LORA_TARGETS = ["qkv_proj", "gate_up_proj", "down_proj"]

# base vocab 
BASE_VOCAB = 32064

print("wake2vec P2: Phi-3.5-mini-instruct LoRA config")
...
tok = AutoTokenizer.from_pretrained(str(P1_SOURCE), use_fast=True, trust_remote_code=True)
...
print(f"  Base vocab (padded matrix): {BASE_VOCAB}")
print(f"  Wake tokens (above padded base): {NUM_WAKE}")
...
# load P1 embeddings into the input embedding layer
...
print(f"[CONFIG CHECK] save_steps={args.save_steps} eval_steps={args.eval_steps} "
      f"logging_steps={args.logging_steps} max_steps={args.max_steps}")
...
print("p2: Phi-3.5-mini-instruct LoRA fine-tune")
print(f"  Train: {len(train_ds)} blocks | Val: {len(val_ds)} blocks")
...
if state_to_use is not None:
    ...
    if train_data:
        ...
        plt.title('Wake2Vec P2: Phi-3.5-mini-instruct LoRA Loss Curve')
        plt.legend(); plt.grid(True, alpha=0.3)
        plot_path = RUN_DIR / "p2_phi35_loss_curve.png"
